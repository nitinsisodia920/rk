...
function Cooler() {
  const navigate = useNavigate();
  const [CoolerProducts, setCoolerProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get('/api/products/cooler');
        setCoolerProducts(result.data);
        setLoading(false);
      } catch (err) {
